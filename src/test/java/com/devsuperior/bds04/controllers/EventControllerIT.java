...
public class EventControllerIT {
	...
	public void insertShouldReturn422WhenAdminLoggedAndPastDate() throws Exception {
		...
		result.andExpect(status().isUnprocessableEntity());
		result.andExpect(jsonPath("$.erros[0].fieldName").value("date"));
		result.andExpect(jsonPath("$.erros[0].message").value("A data do evento não pode ser passada"));
	}
	...
	public void insertShouldReturn422WhenAdminLoggedAndNullCity() throws Exception {
		...
		result.andExpect(status().isUnprocessableEntity());
		result.andExpect(jsonPath("$.erros[0].fieldName").value("cityId"));
		result.andExpect(jsonPath("$.erros[0].message").value("Campo requerido"));
	}
